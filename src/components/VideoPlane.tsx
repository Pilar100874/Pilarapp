...
export const VideoPlane = ({ texturePath }: VideoPlane) => {
  ...
  const initialY = -10; // Start from bottom
  const targetY = 0; // Final position
  ...
  useFrame((state) => {
    ...
    ref.current.position.y = initialY + (targetY - initialY) * progress;

    // Start video when animation is complete
